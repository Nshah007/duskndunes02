...
(function () {
  ...
  /* ---------- Pre-fill "Experience / Activity" from ?package= query param
     (used by tour-packages.html "ENQUIRE NOW" links) ---------- */
  function setupPackageFromQuery() {
    ...
    if (field) field.value = decodeURIComponent(pkg);
  }
  ...
  function setupPackageAutoFill() {
    document.querySelectorAll("[data-package-select]").forEach(function (btn) {
      btn.addEventListener("click", function (e) {
        var form = document.querySelector("#enquiryForm");
        if (!form) return; // link will simply navigate to contact.html instead
        e.preventDefault();
        var field = form.querySelector("#experience");
        if (field) field.value = btn.getAttribute("data-package-select");
        var target = document.querySelector("#enquiry");
        if (target) target.scrollIntoView({ behavior: "smooth", block: "start" });
        var nameField = form.querySelector("#name");
        if (nameField) window.setTimeout(function () { nameField.focus(); }, 500);
      });
    });
  }

  /* Prevent past-date selection in the enquiry form */
